...
__global__ void top_k_kernel(float* scores, int* indices, float* values, int batch_size, int num_experts, int k) {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        scores[b * num_experts + max_idx] = -INFINITY;  // Now valid since `scores` is non-const
    }
}
...
torch::Tensor mixture_of_experts(
    ...
    top_k_kernel<<<blocks, threads_per_block>>>(
        gating_scores.data_ptr<float>(),  // Non-const pointer
